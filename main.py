        ...
        try:
            ...
            for x in mems[0:150]:
                if x not in alip.FindDuplicatedAndRemove(creator, owner, Bots):
                    targk.append(x)
            nazwa = "kickall.js gid={} token={} mod=single".format(
                group,
...
def RECEIVE_MESSAGE(op):
    ...
    try:
        ...
        day = datetime.strftime(timeNow, "%d")
        timess = datetime.strftime(timeNow, "%H:%M")
        trace = catch.__traceback__
        err = (
            "Error Name: "
            + str(trace.tb_frame.f_code.co_name)
            + "\nError Filename: "
            + str(trace.tb_frame.f_code.co_filename)
            + "\nError Line: "
            + str(trace.tb_lineno)
            + "\nError: "
            + str(catch)
